...
# ── Global speed scale — applies to every mode uniformly ────────────────
# A single multiplier applied once, at the point where every mode's
# left/right output converges right before being sent to the motors —
# NOT by touching any individual mode's own tuned constants (STEER_GAIN,
# ACTION_SCALE, CATEGORY_MOTOR_COMMANDS, etc). This keeps each mode's
# relative behavior/tuning intact while proportionally speeding up or
# slowing down all of them the same way. 1.0 = unmodified (100%); 0.5 =
# half speed; 1.5 = 50% faster. Adjustable live with +/- while driving —
# see the key handling in the main loop.
GLOBAL_SPEED_SCALE = 1.0
GLOBAL_SPEED_SCALE_STEP = 0.5
GLOBAL_SPEED_SCALE_MIN = 0.1
GLOBAL_SPEED_SCALE_MAX = 3.0
...
# A median+EMA smoothing variant (mirroring modes 2/3's detection
# smoothing) was tried here on real hardware, specifically to fix an
# observed "slithering" oscillation on approach. It did NOT fix it, and
# was reverted — mode 9 uses the raw per-frame color-detector output
# directly, matching what its actor was actually trained on. Current
# honest read: the oscillation more likely comes from this small ReLU
# network's own piecewise-linear output landscape being less smooth than
# its input, not from input noise — smoothing the input doesn't touch
# that. The oscillation remains a known, open, unresolved issue.
