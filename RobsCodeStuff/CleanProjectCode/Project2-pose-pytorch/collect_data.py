...
Controls:
  ...
  Space   start a timed capture burst — waits PRE_RECORD_DELAY_SEC seconds
          to let you get into pose, then auto-captures at TIMER_CAPTURE_HZ
...
PRE_RECORD_DELAY_SEC  = 3.0  # seconds to get into pose after pressing Space,
                              # before capturing actually starts
...
for i, cls in enumerate(GESTURE_CLASSES):
    print(f"  {i + 1} = {cls}")
print(f"Timed burst: {PRE_RECORD_DELAY_SEC}s to get ready, then {TIMER_CAPTURE_HZ} Hz for {TIMER_DURATION_SEC}s")
