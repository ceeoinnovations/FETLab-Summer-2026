...
**Camera selection** — the script scans for connected cameras, lists them by name, and asks you to choose:
...
Enter camera number to use [0]:
```

Press **Enter** to accept the default (first camera found), or type a number and press **Enter** to pick a different one.

A window then opens showing the webcam feed with the pose skeleton drawn on top and the current motor speeds in the top-left corner. Press **Q** to stop.
