...
# no training, the safe fallback) or "grid" (this project's actual method
# — you must run generate_pseudo_boxes.py then train_grid_detector.py
# first, or this will fail to find a model file). See detect.py's
# docstring. Note: generate_pseudo_boxes.py and calibrate_color.py always
# use the color method regardless of this setting — they exist to
# bootstrap/tune it.
DETECTOR_BACKEND = "grid"
PERCEPTION_MODEL_PATH = "grid_detector_model.pt"
...
FORWARD_MAX_SPEED = 50
FORWARD_SLOWDOWN_AREA = 0.08
STOP_AREA_FRACTION = 0.75
...
SEARCH_TURN_SPEED = 5
SEARCH_REVERSE_DIRECTION_AFTER = 16.0  # seconds before flipping search direction
...
# imitated, instead of a human driving by hand. Ported from
# option1_keypoint_regression's config.py — same format, same behavior,
# just exporting this project's (grid-detector-driven) runs instead.
#
