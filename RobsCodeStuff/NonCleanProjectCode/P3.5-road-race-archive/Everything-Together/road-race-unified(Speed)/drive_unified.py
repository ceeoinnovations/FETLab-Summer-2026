...
from config import (
    ...
    GLOBAL_SPEED_SCALE, GLOBAL_SPEED_SCALE_STEP, GLOBAL_SPEED_SCALE_MIN, GLOBAL_SPEED_SCALE_MAX,
...
class SearchOnlyState:
    ...
    same reflex as modes 2/3's search behavior. (A median+EMA smoothing
    variant was tried on real hardware and did NOT resolve the observed
    oscillation — reverted; mode 9 uses the raw per-frame color-detector
    output directly, matching what its actor was actually trained on.)"""
...
def run_offline_rl(frame_bgr, state):
    ...
    Uses the RAW per-frame reading, no smoothing: a median+EMA smoothing
    variant was tried on real hardware specifically to fix an observed
    "slithering" oscillation, and it did NOT resolve it — reverted.
    Current honest read: the oscillation more likely comes from this
    small ReLU network's own piecewise-linear output landscape being
    less smooth than the input, not from input noise — smoothing the
    input doesn't touch that. The oscillation is a known, open issue.
...
print("Press +/- at any time to speed up/slow down (applies to every mode).")
print("=" * 70)

# ── Mutable per-mode state, reset whenever the active mode changes ──────
mode = 1
speed_scale = GLOBAL_SPEED_SCALE  # persists across mode switches -- NOT reset by reset_mode_state()
state0 = MPCState()
...
try:
    while cap.isOpened():
        ...
        # ── Global speed scale — applies uniformly regardless of which mode
        # is active (see config.GLOBAL_SPEED_SCALE's comment). Clipped back to
        # the motors' actual physical range in case scaling up pushes an
        # already-near-max speed out of bounds, then deadzone re-applied since
        # scaling down can push a previously-valid speed back into it.
        left_speed = max(MOTOR_SPEED_MIN, min(MOTOR_SPEED_MAX, left_speed * speed_scale))
        right_speed = max(MOTOR_SPEED_MIN, min(MOTOR_SPEED_MAX, right_speed * speed_scale))
        ...
        cv2.putText(hud, f"L: {left_speed:+5.1f}  R: {right_speed:+5.1f}   Speed: {speed_scale*100:.0f}%",
        ...
        cv2.putText(hud, "0-9: switch model   +/-: speed   Q: quit", (10, 55),
        ...
        if key == ord('q'):
            ...
            speed_scale = round(min(GLOBAL_SPEED_SCALE_MAX, speed_scale + GLOBAL_SPEED_SCALE_STEP), 2)
            print(f"Speed scale: {speed_scale*100:.0f}%")
        elif key == ord('-'):
            speed_scale = round(max(GLOBAL_SPEED_SCALE_MIN, speed_scale - GLOBAL_SPEED_SCALE_STEP), 2)
            print(f"Speed scale: {speed_scale*100:.0f}%")
