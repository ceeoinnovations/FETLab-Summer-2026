...
> **Note on the overlay:** the class name / counts / instructions text is drawn only on the preview window, on a copy of the frame. The image actually written to disk is the clean, un-overlaid frame — so the UI text never ends up baked into your training data and can't taint the model.

**Workflow:**
1. Press a number key (`1`–`5`) to select a gesture class.
2. Strike that pose in front of the camera.
3. Capture frames using either method below. Repeat until you have at least **30–50 images** for that class.
...
| Key | Behavior |
|---|---|
| **Space** | Starts a timed burst: auto-captures frames at `TIMER_CAPTURE_HZ` for `TIMER_DURATION_SEC` seconds, then stops on its own. A red "RECORDING …s left" indicator shows the countdown. Useful for holding a pose while it collects varied frames of the same gesture for you. |
| **Enter** | Tap once to capture a single frame immediately. Hold it down to keep capturing frames for as long as it's held. |

Both are adjustable at the top of `collect_data.py`:
...
`ENTER_MIN_INTERVAL_SEC` exists because holding Enter down relies on your OS/keyboard's key-repeat feature to keep sending key-presses — OpenCV has no true key-up event, so there's no way to detect "held" directly. This value throttles how fast repeated key events turn into saved frames, so a fast key-repeat rate doesn't flood you with near-duplicate images. Lower it for faster captures, raise it if you're getting too many near-identical frames.

You can't switch classes or start a new Space burst while a timed burst is already running — let it finish (or it'll finish itself after `TIMER_DURATION_SEC`) before doing either.

**Tips for good data:**
- Vary your distance from the camera, lighting, and exact body position. Diversity makes the model more robust.
- Move a little between captures — don't just hold a single frozen pose for an entire Space burst.
