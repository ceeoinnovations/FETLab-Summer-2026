...
Expects a MFE (log-mel spectrogram) transfer-learning model with 4D input: (1, frames, mel_bins, 1).
Model inspection at startup will print the actual shapes so you can verify CONFIG is correct.
"""
...
MODEL_PATH = (
    ...
    "(keyword-spotting)-tensorflow-lite-float32-model.9.lite"
)
...
INFERENCE_STRIDE_MS  = 200    # ms between inferences (latency vs CPU trade-off)
DEBOUNCE_MS          = 1500   # ms to ignore repeated detections of the same keyword
...
def extract_features(audio_int16, filterbank):
    """
    audio_int16: int16 array of shape (AUDIO_SAMPLES,)
    Returns float32 array of shape (1, N_FRAMES, N_MEL_FILTERS, 1)
    """
    ...
    return log_mel.flatten()[np.newaxis, :]   # (1, N_FRAMES * N_MEL_FILTERS)


def run_inference(interp, input_detail, output_detail, features):
    interp.set_tensor(input_detail['index'], features)
    interp.invoke()
    return interp.get_tensor(output_detail['index'])[0]   # (n_classes,)


# Ring buffer filled by the sounddevice callback on the audio thread
...
def start(callbacks=None):
    """
    Start the recognition loop.

    callbacks: dict mapping label name to a callable, e.g.
        {"Go": my_go_fn, "Stop": my_stop_fn}
    Each callable is called as fn(label, score) when that keyword is detected.
    Labels not in callbacks still appear in the console output but trigger no function.
    Pass None (or omit) to run in print-only mode.
    """
    ...
    last_detected = {}   # label -> time.monotonic() of last fired callback
    ...
    with sd.InputStream(
        ...
        while True:
            ...
            detected = [
                ...
                and scores[i] >= THRESHOLDS.get(LABELS[i], 0.70)
                and now - last_detected.get(LABELS[i], 0) >= DEBOUNCE_MS / 1000
            ]
            if detected:
                print(f"DETECTED: {', '.join(l for l, _ in detected)}  |  {bar_str}")
                for label, score in detected:
                    last_detected[label] = now
                    if label in callbacks:
                        callbacks[label](label, score)
            else:
