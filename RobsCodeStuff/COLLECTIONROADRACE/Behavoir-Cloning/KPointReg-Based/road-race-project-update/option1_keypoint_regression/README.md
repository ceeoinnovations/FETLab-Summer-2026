...
## Exporting this project's driving as training data for another project

`drive.py` can optionally save every frame it drives on, plus the motor
command it actually computed and sent that frame, in the exact same
format `collect_data.py` produces — turning this project's own
perception+control loop into an "expert" whose behavior gets recorded
as if a very consistent, low-noise human were driving.

To use this: set `EXPORT_EXPERT_DATA = True` in `config.py`, then run
`drive.py` as normal. It drives exactly as it always did, and also
writes to `expert_data/images/` + `expert_data/labels.csv` (filename,
left_speed, right_speed, session_id) at `EXPERT_CAPTURE_HZ` (default
10Hz, independent of the ~30Hz control loop, to avoid saving many
near-duplicate frames). Re-running `drive.py` continues numbering
frames and starts a new `session_id` automatically, same as
`collect_data.py` always did.

This is exactly the data format `road-race-expert-data` expects — copy
`expert_data/images` and `expert_data/labels.csv` into that project's
`data/` folder and train directly, no conversion needed.
