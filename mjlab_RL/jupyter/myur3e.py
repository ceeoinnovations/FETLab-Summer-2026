...
class MyUR3e(rclpy.node.Node):
    ...
    def __init__(
        ...
        ######################## ADDED LINE TO FIX OVERLAPPING COMMANDS / GENERATOR ISSUE ##################
    ...
    def read_global_pos(self,degrees=True,euler=True):
        """
        ...
        return self.solve_fk(
            self.read_joints_pos(degrees=degrees), degrees=degrees, euler=euler
        )
    ...
    def move_joints(
        ...
        message = "joints: " + str(joint_positions)
        self.get_logger().info(message)
        message = "time: " + str(time)
        self.get_logger().info(message)
        """
...
#        self.done = True
    ...
    def spin_async(self, curr_id):
        """
        Updated spin_async to work with overlapping commands
        """
        def check_id():
            if curr_id is not self._id:
                self.get_logger().info(
