"""Run the trained train13 policy on the REAL LEGO quadruped.

    python run_on_robot13.py   (hardware env; pip install legoeducation numpy)

Copy these three files together onto the hardware machine:
    run_on_robot13.py, numpy_policy.py, policy_weights13.npz

train13 is the SAME setup as train12 (phase clock, two-critic, RSI,
translation-dominant reward) trained to walk the OPPOSITE way - your robot's
ORIGINAL intended direction. In sim it walks ~0.026 m/s (net ~-0.8 m/30 s in
body -x), cold-starts from rest, clean diagonal trot. Its dominant crank
pattern is [-,+,-,+] (the exact opposite of train12's [+,-,+,-]) - so the walk
DIRECTION is baked into the policy's actions, and this script uses the SAME
LEG_SIGNS=[+1,+1,+1,+1] and LEG_MAP as run_on_robot12; only the weights differ.
Deploy train12's weights to walk the other way.

Notes:

1. PHASE CLOCK IN THE OBSERVATION (obs is 28-dim). The trot clock (sin/cos,
   CLOCK_T=0.72 s) advances on wall-clock time from start, as in training.

2. DIRECTION. train13 walks your original forward. If it walks backward on
   hardware, flip ALL four LEG_SIGNS; VERIFY SLOWLY FIRST (low MAX_SPEED_PCT).

BEFORE trusting it, sanity-check LEG_MAP / LEG_SIGNS with scripted_trot_test.py
on THIS robot (one leg fights -> that leg's LEG_MAP back-unit L/R swap). The
run_on_robot12 sign fix ([+1,+1,+1,+1]) was verified against your observed crank
directions; the same applies here since only the policy weights change.
