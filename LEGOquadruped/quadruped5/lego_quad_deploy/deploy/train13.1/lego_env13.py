...
class LegoQuadEnv(gym.Env):
  ...
  def __init__(
    ...
    self.observation_space = gym.spaces.Box(-np.inf, np.inf, (28,), np.float64)
  ...
  def reset(self, seed=None, options=None):
    ...
    if self.randomize:
      # light domain randomization: floor friction only. NO random crank
  ...
  def step(self, action):
    ...
    balance = -0.3 * float(np.std(crank_speed))
    heading = -1.0 * abs(self._wrap_pi(self._yaw() - self._yaw0))
