"""Run the trained train15 policy on the REAL LEGO quadruped.

    python run_on_robot15.py   (hardware env; pip install legoeducation numpy)

Copy these three files together onto the hardware machine:
    run_on_robot15.py, numpy_policy.py, policy_weights15.npz

train15 = train13's original direction + HEADING ERROR IN THE OBSERVATION, so
the policy STEERS ITSELF instead of relying on an external heading-hold trim.
Same crank convention as run_on_robot13 (LEG_SIGNS=[+1,+1,+1,+1], LEG_MAP), same
translation-dominant walk in the original direction.
...
MASK = {"left": 1, "right": 2}
policy = NumpyPolicy("policy_weights15.npz")
