...
class LegoQuadEnv(gym.Env):
  ...
  def step(self, action):
    ...
    # anti-exploit terms (policies were pinning front cranks at max):
    # saturation: any |action| past 0.9 costs, up to -0.8 if all four pinned
    sat = -2.0 * float(np.mean(np.clip(np.abs(action) - 0.9, 0.0, None)))
    # leg balance: all four cranks should share the work -> penalize spread
    # of per-leg |speed| across legs (rad/s scale, ~0 for an even trot)
    crank_speed = np.abs(self.data.qvel[self.crank_qvel])
    balance = -0.08 * float(np.std(crank_speed))
    reward = 2.0 * track + lateral + upright + smooth + sat + balance
