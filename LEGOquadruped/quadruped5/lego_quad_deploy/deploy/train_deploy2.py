"""Train the DEPLOYMENT policy (hardware-matched observations, 20 Hz).
    ...
    uv run python train_deploy2.py
    Has longer episode length than train_deploy.py

Produces lego_quad_deploy_ppo4.zip. Run export_policy.py afterwards.
"""
...
MODEL_PATH = Path("lego_quad_deploy_ppo4.zip")


def make_env():
  return LegoQuadDeployEnv(episode_s=30.0)
