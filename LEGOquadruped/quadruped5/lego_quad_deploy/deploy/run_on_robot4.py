...
Needs policy_weights3.npz (from export_policy.py) and numpy_policy.py
...
CONTROL_HZ = 20.0
MAX_CRANK_SPEED = 12.0  # rad/s, must match training
...
policy = NumpyPolicy("policy_weights3.npz")
...
def main():
  ...
  time.sleep(0.5)
  pitch0 = math.radians(imu.pitch)
  roll0 = math.radians(imu.roll)
  ...
  try:
    while True:
      ...
      pitch = wrap(math.radians(imu.pitch) - pitch0)
      roll = wrap(math.radians(imu.roll) - roll0)
      ...
      gyro = np.radians([imu.gyroscopeX, imu.gyroscopeY, imu.gyroscopeZ])
      ...
      ang = np.array([math.radians(u.motor[m].absolutePosition) for u, m in legs])
      ang = (ang - ang0) * LEG_SIGNS
      vel = np.array([u.motor[m].speed / 100.0 * MAX_DEG_S for u, m in legs])
      vel = np.radians(vel) * LEG_SIGNS
