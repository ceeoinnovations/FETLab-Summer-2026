...
class LegoQuadEnv(gym.Env):
  ...
  def __init__(
    self,
    cmd_range=(0.03, 0.15),  # forward speed commands sized for the 12 cm robot
  ...
  def step(self, action):
    ...
    # --- reward ---
    track = np.exp(-((v_b[0] - self.cmd) / 0.08) ** 2)      # follow the command
    lateral = -0.5 * abs(v_b[1]) - 0.3 * abs(w_b[2])        # go straight
    upright = -0.5 * tilt                                     # stay level
    smooth = -0.05 * float(np.sum((action - self.prev_action) ** 2))
    # anti-exploit terms, v2 - previous coefficients lost to the tracking
    # reward and the policy still pinned 3 of 4 cranks at max:
    # (a) hard saturation: |action| past 0.8 costs steeply
    sat = -6.0 * float(np.mean(np.clip(np.abs(action) - 0.8, 0.0, None)))
    # (b) energy: quadratic cost on action magnitude discourages living at
    #     the rail even below the saturation threshold
    energy = -0.3 * float(np.mean(action ** 2))
    # (c) leg balance: four cranks share the work (stronger than v1)
    crank_speed = np.abs(self.data.qvel[self.crank_qvel])
    balance = -0.15 * float(np.std(crank_speed))
    # (d) HEADING: pay for accumulated yaw, not just yaw rate. A slow 1
    #     deg/s bias is invisible to the |wz| penalty but adds up to a
    #     30-degree arc per episode; |yaw| charges for the arc itself.
    q = self.data.qpos[3:7]
    yaw = np.arctan2(2 * (q[0] * q[3] + q[1] * q[2]),
                     1 - 2 * (q[2] ** 2 + q[3] ** 2))
    heading = -0.6 * abs(yaw)
    reward = 2.0 * track + lateral + upright + smooth + sat + energy + balance + heading
