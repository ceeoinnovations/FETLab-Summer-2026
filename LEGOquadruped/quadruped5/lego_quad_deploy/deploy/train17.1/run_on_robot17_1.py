"""Run the trained train17.1 policy on the REAL LEGO quadruped.

    python run_on_robot17_1.py   (hardware env; pip install legoeducation numpy)

Copy these three files together onto the hardware machine:
    run_on_robot17_1.py, numpy_policy.py, policy_weights17.npz

train17.1 = train15's heading-aware self-steering walk (original direction) plus a
ONE-SIDED front-back balance reward (penalize front driving LESS than back) and
realistic, surface-RANDOMIZED rubber-foot friction (mu ~1.4, sampled 0.6-1.8).
The one-sided penalty leaves a front-bias loophole, so in sim it drives the FRONT
cranks harder than the back (front |drive| 8.7 >> back ~4.3) - it walks and steers
(~6-9 deg veer, ~0.026 m/s) but is NOT the even four-leg gait; train18 (two-sided
balance) is the recommended deploy pick. Keep this as the friction-randomized
one-sided comparison. Same crank convention as run_on_robot15/18
(LEG_SIGNS=[+1,+1,+1,+1], LEG_MAP), same 30-dim heading-in-obs.
...
5. FRONT-BIASED GAIT: unlike train18, this policy drives the front cranks harder
   than the back by design (one-sided balance), so a lightly-driven back leg is
...
MASK = {"left": 1, "right": 2}
policy = NumpyPolicy("policy_weights17.npz")
