"""Run the trained train5.1 policy on the REAL LEGO quadruped.
...
IMPORTANT - this policy REQUIRES live IMU. train5.1 was trained with
imu_mode="live" (real gyro + gravity). In sim eval it walks forward
(+1.64 m / 30 s) with live IMU but walks BACKWARD (-0.47 m) when fed a
frozen/zero IMU. So keep USE_LIVE_IMU = True for this policy - unlike the
earlier train4 policy, frozen-IMU mode here makes it go the wrong way.
It also curves (~-66 deg / 30 s in sim even with IMU), so HEADING_HOLD is
on to steer it straight with differential leg speed.
...
USE_LIVE_IMU = True   # True: feed real accelerometer-gravity + gyro.
                      # train5.1 REQUIRES this (trained on live IMU) - False
                      # makes it walk backward. Do not A/B this one to False.
