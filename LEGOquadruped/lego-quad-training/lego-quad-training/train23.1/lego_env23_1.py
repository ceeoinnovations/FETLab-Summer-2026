...
GAIT_GATE = -0.6                # (unused in v19; kept for reference)
GAIT_WEIGHT = 2.5               # barrier scale (washes out under normalization)
# v19: the gait reward is now CLOCK-FREE diagonal-trot coordination (see
# _gait_coord), gated off below this crank speed so slow/paused/steering gaits
# are not penalized. Enforce STRENGTH via TwoCriticPPO(barrier_adv_coef=...).
GAIT_ACTIVE_SPEED = 4.0         # rad/s: coordination reward fully on above this
FWD_GATE_SPEED = 0.015          # m/s: v23 - coordination only paid while translating
                                # this fast (prevents strong coord -> trot-in-place)
...
RSI_PROB = 0.6                  # v23.1: fewer seeded starts (was 0.85) - also learn
                                # to start from rest / a broken state
RSI_WARMUP_CYCLES = 2.0         # crank revolutions to reach steady gait pre-policy
# v23.1 RECOVERY: seed some episodes with a BROKEN (co-rotating) pair and inject
# occasional mid-episode phase kicks, so the policy learns to RE-separate a synced
# front/back pair, not just avoid syncing. The back-active balance (BACK_MIN_FRAC)
# keeps it from collapsing to front-only the way train19.1 did.
RSI_BREAK_PROB = 0.5            # of seeded episodes, fraction started de-synced
KICK_PROB = 0.02               # per-step chance of a crank velocity kick
KICK_MAG = 6.0                 # rad/s magnitude of the kick
...
FB_WEIGHT = 0.4
BACK_MIN_FRAC = 0.5     # v23: back drive must be >= this fraction of front drive
                        # (front-bias OK, but back never idle -> all four move)
...
class LegoQuadEnv(gym.Env):
  ...
  def _gait_coord(self):
    """Clock-FREE diagonal-trot coordination in [-1, 1], gated toward 0 when the
    cranks are barely moving. +1 = ideal trot structure: the two diagonals
    (FL+BR, FR+BL) share a contact state and every other pair is opposed. This
    rewards the gait STRUCTURE, not keeping pace with the global clock, so the
    policy is free to slow down, pause, or slow one side to steer without losing
    reward - only an actual loss of coordination (e.g. the two front feet
    syncing into the same contact state) is penalized. v19: replaces the
    clock-locked contact barrier, which punished exactly those free behaviours."""
    c = self._foot_contacts() > 0.5
    inphase = ((0, 3), (1, 2))                     # diagonals move together
    antiphase = ((0, 1), (2, 3), (0, 2), (1, 3))   # front, back, both sides oppose
    s = 0.0
    for i, j in inphase:
      s += 1.0 if c[i] == c[j] else -1.0
    for i, j in antiphase:
      s += 1.0 if c[i] != c[j] else -1.0
    speed = float(np.mean(np.abs(self.data.qvel[self.crank_qvel])))
    gate = min(1.0, speed / GAIT_ACTIVE_SPEED)     # pauses/slow gaits: reward ~0
    # v23 FORWARD gate: only pay for coordination while actually TRANSLATING, so a
    # strong coordination weight can't be farmed by trotting in place (the train19
    # coef=2.0 failure). Zero when stalled/backward -> the policy must move forward
    # to earn any structure reward.
    _, v_b, _ = self._body_frame()
    fgate = min(1.0, max(0.0, (FWD_SIGN * v_b[0]) / FWD_GATE_SPEED))
    return (s / 6.0) * gate * fgate
  ...
  def reset(self, seed=None, options=None):
    ...
    if self.np_random.uniform() < RSI_PROB:
      ...
      # v23.1 recovery: on some seeds, flip ONE pair to co-rotate so the episode
      # starts in the synced (bad) state the policy must learn to break out of.
      signs = RSI_SIGNS.copy()
      if self.np_random.uniform() < RSI_BREAK_PROB:
        pair = (0, 1) if self.np_random.uniform() < 0.5 else (2, 3)
        signs[pair[1]] = signs[pair[0]]           # both cranks now drive one way
      for _ in range(n_pre):
        self.data.ctrl[:] = signs * omega
        mujoco.mj_step(self.model, self.data)
  ...
  def step(self, action):
    ...
    # v23.1 recovery: occasionally kick one crank's velocity mid-episode so the
    # policy must re-coordinate a disturbed gait, not just sustain a clean one.
    if self.np_random.uniform() < KICK_PROB:
      k = int(self.np_random.integers(0, 4))
      self.data.qvel[self.crank_qvel[k]] += self.np_random.uniform(-1.0, 1.0) * KICK_MAG
    ...
    # v23 front-back balance: front-bias ALLOWED, but the back legs must never go
    # idle (train19.1/21 collapsed to front-only). Penalize BOTH back>front (keep
    # the front bias) AND back < BACK_MIN_FRAC*front (keep the back driving). Net:
    # BACK_MIN_FRAC*front <= back <= front - all four move, a little front-biased.
    ...
    balance = -FB_WEIGHT * (max(0.0, float(back_drive - front_drive))
                            + max(0.0, float(BACK_MIN_FRAC * front_drive - back_drive)))
    ...
    r_barrier = float(GAIT_WEIGHT * self._gait_coord())
