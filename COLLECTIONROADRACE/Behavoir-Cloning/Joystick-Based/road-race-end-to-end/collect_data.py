...
from config import CAMERA, SERIAL, CAPTURE_HZ, MOTOR_SPEED_MIN, MOTOR_SPEED_MAX, MOTOR_DIVIDE_BY, apply_deadzone
...
with open(LABELS_CSV, "a", newline="") as f:
    ...
    try:
        while True:
            ...
            left_speed, right_speed = round(js.left_position()/MOTOR_DIVIDE_BY), round(js.right_position()/MOTOR_DIVIDE_BY)
            ...
            # (deadzone applied here only — the label recorded above stays raw)
            dm.movement_move_tank(apply_deadzone(left_speed)/10, apply_deadzone(right_speed)/10)
